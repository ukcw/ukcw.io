  ...
  return (
    <div
      className={`px-5 py-5 lg:mx-auto lg:py-9 max-w-7xl flex justify-between ${
        isOpenBurger ? 'dark:bg-gray-800' : 'transparent'
      }`}
    >
      <nav className="mx-auto flex flex-row justify-between max-w-3xl lg:max-w-full w-full items-center">
              ...
              case 'component': {
                return (
                  <li className="px-5 py-2" key={item.title}>
                    <Link
                      to={`${item.route}`}
                      className={`block text-lg whitespace-nowrap font-medium hover:underline focus:underline focus:outline-none active ${
                        pathname === item.route
                          ? 'text-current-focus'
                          : 'text-secondary'
                      }  underline-offset-8`}
              ...
              case 'pdf': {
                return (
                  <li className="px-5 py-2" key={item.title}>
                    <Link
                      reloadDocument
                      to={item.route} // hard-coded path for fix - Link component does not work
                      className={`block text-lg whitespace-nowrap font-medium hover:underline focus:underline focus:outline-none active ${
                        pathname === item.route
                          ? 'text-current-focus'
                          : 'text-secondary'
        ...
        <button
          className="px-2 py-2 hidden lg:block border-gray-200 dark:border-gray-500  rounded-full border-2 hover:border-black hover:dark:border-white"
          onClick={toggleTheme}
        >
          <div className="">
            {themeValue === 'dark' ? (
              <FaRegMoon className="h-5 w-5 text-white " />
            ) : (
              <FaSun className="h-5 w-5 text-black " />
            )}
          </div>
        </button>
      ...
      {isOpenBurger && (
        <ul className="absolute top-12 left-0 z-50 min-h-full w-full px-8 py-8 bg-white lg:hidden shadow-gray-800/80 bg-opacity-100 dark:bg-gray-800">
          {navLinks.map((item) => {
            switch (item.type) {
              case 'component': {
                return (
